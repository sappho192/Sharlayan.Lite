        ...
        public static ChatLogItem Process(byte[] raw) {
            ...
            try {
                ...
                if (Constants.ChatPublic.Contains(chatLogEntry.Code)) {
                    int separatorIndex = chatLogEntry.Line.IndexOf(INDEX_CHECK, StringComparison.OrdinalIgnoreCase);
                    if (separatorIndex >= 0) {
                        chatLogEntry.PlayerName = chatLogEntry.Line.Substring(0, separatorIndex);
